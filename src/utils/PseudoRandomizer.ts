/*
* @see https://github.com/zeh/prando
*/
export class PseudoRandomizer {
  ...
  /**
   * Generate a new PseudoRandom pseudo-random number generator.
   *
  ...
  constructor(seed?: number | string) {
    if (typeof seed === "string") {
      ...
      // Pseudo-random seed
      this._seed = this.getSafeSeed(PseudoRandomizer.MIN + Math.floor((PseudoRandomizer.MAX - PseudoRandomizer.MIN) * Math.random()));
    }
    this.reset();
  }
   ...
   * @param max - The maximum number that can be randomly generated (exclusive).
   * @return The generated pseudo-random number.
   */
  public next(min = 0, max = 1): number {
    this.recalculate();
    return this.map(this._value, PseudoRandomizer.MIN, PseudoRandomizer.MAX, min, max);
  }
  ...
  public nextInt(min = 10, max = 100): number {
    this.recalculate();
    return Math.floor(this.map(this._value, PseudoRandomizer.MIN, PseudoRandomizer.MAX, min, max + 1));
  }
   ...
   * creating a new PseudoRandom instance with the same seed as another PseudoRandom instance.
   *
   * Example:
   * let rng = new PseudoRandom(12345678);
